...
dall'admin prodotto per prodotto: tutto il catalogo in un'unica tabella, modifica singola
o in blocco, sistemazione di taglie/varianti, pulizia immagini, raggruppamento prodotti e
accesso condiviso con un socio.

Nato per negozi gestiti da una o due persone (niente SaaS multi-tenant, niente database,
...
- **Taglie / varianti** — crea, rinomina, riordina ed elimina opzioni e varianti, per prodotto.
- **Immagini** — riordina ed elimina le immagini dei prodotti.
- **Metafield** — legge/scrive `custom.fornitore_url`, `custom.modello`, `custom.gruppo`
  (usati dalla vista *Gruppi*).
- **Vista Gruppi** — suggerimenti automatici di raggruppamento (stesso modello, colori diversi).
...
- **Node.js 20+** (oppure Docker, vedi sotto)
...
Apri <http://localhost:3000> e accedi con `CATALOG_USER_1_ID` / `CATALOG_USER_1_PASSWORD`
impostati in `.env.local`.

> Su macOS puoi anche fare doppio click su **`Avvia CatalogFlow.command`**: al primo avvio
> installa le dipendenze, avvia il server e apre il browser.

## 1. Creare l'app Shopify (una volta)
...
Guida completa con screenshot (13 passaggi, ~15 minuti):
...
La sessione dura 30 minuti, poi richiede di nuovo la password.
...
Apri <http://localhost:3000>. L'immagine costruisce un server Next.js standalone: non serve
Node sull'host e gira su qualsiasi macchina con Docker (NAS, VPS, portatile vecchio).

## Deploy (opzionale)

`scripts/deploy.sh` deploya su Vercel e carica le variabili da `.env.local` sul progetto
(richiede `vercel login` una volta). Qualsiasi host che esegua Node o Docker va bene allo
stesso modo — basta impostare le stesse variabili d'ambiente.
...
- Il token dell'Admin API viene richiesto con il **client credentials grant**
  (`POST /admin/oauth/access_token`), messo in cache 24 h e rinnovato da solo — niente da
  copiare o rigenerare.
- Versione Admin API: **2025-01**. Il throttling GraphQL è gestito con retry basati sul costo.
- Il login è un cookie HMAC firmato (Web Crypto), senza database di sessioni.

## Struttura del progetto

```
src/app/            pagine (catalogo, login) + route API
src/lib/catalog/    client Shopify, normalizzazione catalogo, gruppi, taglie, tassonomia
src/components/     tabella catalogo, dialoghi prodotto, vista gruppi, tray selezione
...
| La pagina di login si ricarica senza entrare | `SESSION_SECRET` mancante o `ID`/`PASSWORD` che non combaciano |
| Catalogo vuoto | store sbagliato, o versione pubblicata senza `read_products` |

Altri casi: [docs/SHOPIFY-SETUP.it.md](docs/SHOPIFY-SETUP.it.md#problemi-frequenti).

## Licenza

MIT — vedi [LICENSE](LICENSE). Sviluppato da [@denchesss-developer](https://github.com/denchesss-developer).
