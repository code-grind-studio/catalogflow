...
or dozens at once, fix sizes/variants, clean up images, group products and share access
with a partner.
...
- **Sizes / variants** — create, rename, reorder and delete options and variants, per product.
- **Images** — reorder and delete product media.
- **Metafields** — the app reads/writes `custom.fornitore_url`, `custom.modello`,
  `custom.gruppo` (used by the *Groups* view).
- **Groups view** — automatic suggestions for grouping products (same model, different colorways).
- **Multi-user, password only** — up to 10 accounts defined in env vars, no emails, no database.
- **Activity log** — who changed what, when (optional Redis persistence).

## Requirements

- **Node.js 20+** (or Docker, see below)
...
Open <http://localhost:3000> and log in with the `CATALOG_USER_1_ID` /
...
Shopify **Dev Dashboard**, install it on your store, and paste the two values into
`.env.local`.

Full walkthrough with screenshots (13 steps, ~15 minutes):
...
| `CATALOG_USER_1_LABEL` | no | name shown in the UI (defaults to the ID) |
...
Sessions last 30 minutes, then the password is asked again.
...
Open <http://localhost:3000>. The image builds a Next.js standalone server — no Node
installation needed on the host, and it runs on any machine with Docker (NAS, VPS, old laptop).

## Deploy (optional)

`scripts/deploy.sh` deploys to Vercel and pushes the variables from `.env.local` to the
project (requires `vercel login` once). Any host that runs Node or Docker works the same —
...
- The Admin API token is obtained with the **client credentials grant**
  (`POST /admin/oauth/access_token`), cached for 24 h and renewed automatically — nothing
  to copy or rotate.
...
src/lib/catalog/    Shopify client, catalog normalisation, groups, sizes, taxonomy
src/components/     catalog table, product dialogs, groups view, selection tray
...
| Login page reloads without entering | `SESSION_SECRET` missing or `<ID>`/`<PASSWORD>` mismatch |
| Empty catalog | wrong store, or the version published without `read_products` |

More cases: [docs/SHOPIFY-SETUP.md](docs/SHOPIFY-SETUP.md#troubleshooting).

## License

MIT — see [LICENSE](LICENSE). Built by [@denchesss-developer](https://github.com/denchesss-developer).
