...
ou en masse, gestion des tailles/variantes, nettoyage des images, regroupement des produits
et accès partagé avec un associé.

Conçue pour les boutiques tenues par une ou deux personnes (pas de SaaS multi-tenant, pas de
base de données, pas de publication sur l'App Store Shopify) — elle tourne sur votre machine
ou votre serveur.
...
- **Tailles / variantes** — créer, renommer, réordonner et supprimer options et variantes, par produit.
- **Images** — réordonner et supprimer les médias des produits.
- **Metafields** — lit/écrit `custom.fornitore_url`, `custom.modello`, `custom.gruppo`
  (utilisés par la vue *Groupes*).
- **Vue Groupes** — suggestions automatiques de regroupement (même modèle, coloris différents).
- **Multi-utilisateur par mot de passe** — jusqu'à 10 comptes définis dans les variables
  d'environnement, sans e-mail ni base de données.
- **Journal d'activité** — qui a modifié quoi et quand (persistance Redis en option).

## Prérequis

- **Node.js 20+** (ou Docker, voir plus bas)
...
```bash
git clone https://github.com/denchesss-developer/catalogflow.git
cd catalogflow
npm install
...
Ouvrez <http://localhost:3000> et connectez-vous avec `CATALOG_USER_1_ID` /
`CATALOG_USER_1_PASSWORD` définis dans `.env.local`.

> Sur macOS vous pouvez aussi double-cliquer sur **`Avvia CatalogFlow.command`** : au premier
> lancement il installe les dépendances, démarre le serveur et ouvre le navigateur.

## 1. Créer l'app Shopify (une fois)
...
Guide complet avec captures d'écran (13 étapes, ~15 minutes) :
...
La session dure 30 minutes, puis le mot de passe est redemandé.
...
Ouvrez <http://localhost:3000>. L'image construit un serveur Next.js standalone : aucune
installation de Node sur l'hôte, et elle tourne sur toute machine avec Docker (NAS, VPS,
vieux portable).

## Déploiement (optionnel)

`scripts/deploy.sh` déploie sur Vercel et pousse les variables de `.env.local` vers le projet
(nécessite `vercel login` une fois). Tout hébergeur qui exécute Node ou Docker fonctionne
pareil — il suffit d'y définir les mêmes variables d'environnement.
...
- Le jeton de l'Admin API est demandé via le **client credentials grant**
  (`POST /admin/oauth/access_token`), mis en cache 24 h et renouvelé automatiquement — rien à
  copier ni à faire tourner.
- Version de l'Admin API : **2025-01**. Le throttling GraphQL est géré avec des retries
  basés sur le coût.
- La connexion est un cookie HMAC signé (Web Crypto), sans base de sessions.

## Structure du projet

```
src/app/            pages (catalogue, connexion) + routes API
src/lib/catalog/    client Shopify, normalisation du catalogue, groupes, tailles, taxonomie
src/components/     tableau catalogue, dialogues produit, vue groupes, tray de sélection
...
| La page de connexion se recharge sans entrer | `SESSION_SECRET` manquant ou `ID`/`PASSWORD` qui ne correspondent pas |
| Catalogue vide | mauvaise boutique, ou version publiée sans `read_products` |

Autres cas : [docs/SHOPIFY-SETUP.fr.md](docs/SHOPIFY-SETUP.fr.md#dépannage).

## Licence

MIT — voir [LICENSE](LICENSE). Développé par [@denchesss-developer](https://github.com/denchesss-developer).
